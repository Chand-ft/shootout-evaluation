...
@Component({
  selector: 'bh-travel_requests',
  templateUrl: './travel_requests.template.html',
  providers: [
    //appendnew_element_providers
  ],
})
export class travel_requestsComponent {
  @ViewChild(MatSort)
  ...
  getAllRequests(...others) {
    try {
      ...
      bh.input = {};
      bh.local = {};
      bh = this.sd_a5K04ZNSIzEmwjyO(bh);
  ...
  //appendnew_flow_travel_requestsComponent_start
  ...
  sd_a5K04ZNSIzEmwjyO(bh) {
    try {
      bh = this.sd_Nj7JYJQRKun10fTt(bh);
      //appendnew_next_sd_a5K04ZNSIzEmwjyO
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_a5K04ZNSIzEmwjyO');
    }
  }
  ...
  async sd_Nj7JYJQRKun10fTt(bh) {
    try {
      ...
      let outputVariables = await callServerApisInstance.dynamicRequest();
  ...
  //appendnew_flow_travel_requestsComponent_Catch
}
